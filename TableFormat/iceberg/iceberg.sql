...
CREATE or replace PERSISTENT secret polaris_secret (
  TYPE ICEBERG,
  CLIENT_ID 'cccccccccccc',
  CLIENT_SECRET 'dddd',
  OAUTH2_SCOPE 'PRINCIPAL_ROLE:data_engineer',
  OAUTH2_SERVER_URI 'https://vvvvvvvvvv.fffffff.com/polaris/api/catalog/v1/oauth/tokens',
  ENDPOINT 'https://vvvvvvvvvv.fffffff.com/polaris/api/catalog'
);


------------------------------------------------------------- 
-- Create and use the Iceberg catalog and schema

ATTACH  or replace 'dwh' AS dwh (
        TYPE iceberg,
...
-- Only rebuild summary if we have new staging data
DELETE FROM summary WHERE EXISTS
 (SELECT 1 FROM scada_staging LIMIT 1) AND EXISTS (SELECT 1 FROM price_staging LIMIT 1);

insert into  summary BY NAME 
select * from summary_staging 
WHERE EXISTS 
(SELECT 1 FROM scada_staging LIMIT 1) AND EXISTS (SELECT 1 FROM price_staging LIMIT 1);
